...
trait SearchController extends BaseController {
  ...
  def search(query: String, pageResults: Option[Int] = None, page: Option[Int]): Action[AnyContent] = Action.async{
    implicit request =>
      val results = lookupService.search(query, pageResults, page)
